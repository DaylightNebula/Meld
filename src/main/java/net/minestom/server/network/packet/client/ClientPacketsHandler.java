...
import net.minestom.server.network.ObjectArray;
...
public sealed class ClientPacketsHandler permits ClientPacketsHandler.Status, ClientPacketsHandler.Login, ClientPacketsHandler.Play {
    private final ObjectArray<Function<NetworkBuffer, JavaClientPacket>> suppliers = ObjectArray.singleThread(0x10);

    private ClientPacketsHandler() {
    }

    public void register(int id, @NotNull Function<@NotNull NetworkBuffer, @NotNull JavaClientPacket> packetSupplier) {
        this.suppliers.set(id, packetSupplier);
    }

    public @UnknownNullability JavaClientPacket create(int packetId, @NotNull NetworkBuffer reader) {
        final Function<NetworkBuffer, JavaClientPacket> supplier = suppliers.get(packetId);
        if (supplier == null)
