...
class ClientConfigurationRegistryData(
    ...
    companion object: JavaPacket.Creator<ClientConfigurationRegistryData> {
        ...
        override fun decode(reader: AbstractReader): ClientConfigurationRegistryData = ClientConfigurationRegistryData(
			registryId = reader.readString(),
			entryId = reader.readArray { reader.readString() },
			data = reader.readOptional { reader.readNBT() }
		)
    }
    ...
    override fun encode(writer: ByteWriter) {
		...
		writer.writeArray(entryId) { entryId -> writer.writeString(entryId) }
		writer.writeOptional(data) { data -> writer.writeNBT(data) }
	}
}
