...
import io.github.daylightnebula.meld.server.generated.JavaClientPlayUnloadChunk
import io.github.daylightnebula.meld.server.generated.JavaClientPlayUpdateViewPosition
...
import io.github.daylightnebula.meld.server.world.chunks.ChunkRegistry
import io.github.daylightnebula.meld.server.world.chunks.getChunkPosition
import io.github.daylightnebula.meld.server.world.chunks.toSectionPosition
...
class Dimension(
    ...
    internal fun unloadChunkForPlayer(player: Player, chunk: Chunk) {
        ...
        // unload chunk
        when (connection) {
            is JavaConnection -> connection.sendPacket(JavaClientPlayUnloadChunk(chunk.position.y.toInt(), chunk.position.x.toInt()))
        }
...
//            is JavaConnection -> (player.connection as JavaConnection).sendPacket(JavaChunkPacket(chunk))
            is JavaConnection -> player.connection.sendPacket(JavaClientPlayMapChunk(
                x = chunk.position.x.toInt(),
                z = chunk.position.y.toInt(),
                heightmaps = ChunkRegistry.defaultHeightmap,
                chunkData = ByteWriter(0, DataPacketMode.JAVA).apply { chunk.writeJava(this) }.getRawData(),
...
//            })
    ...
    internal fun centerPacket(player: Player) {
        ...
        when (player.connection) {
            is JavaConnection -> {
                player.connection.sendPacket(JavaClientPlayUpdateViewPosition(chunkPosition.x.toInt(), chunkPosition.y.toInt()))
            }
    ...
    fun getChunksInViewDistanceOfChunk(chunkPos: Float2): MutableList<Chunk> {
        ...
        ((chunkPos.x - Meld.config.viewDistance).roundToInt() .. (chunkPos.x + Meld.config.viewDistance).roundToInt()).forEach { x ->
            ((chunkPos.y - Meld.config.viewDistance).roundToInt() .. (chunkPos.y + Meld.config.viewDistance).roundToInt()).forEach { y ->
